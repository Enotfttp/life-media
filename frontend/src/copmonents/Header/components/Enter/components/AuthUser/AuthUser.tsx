...
import {useGetFirstStatusOrders} from 'src/rest-api/order/hooks';
...
export const AuthUser = ({userId, setUserId}: {userId: string, setUserId: (id: string | null) => void}) => {
  ...
  return (
    <>
      <IconButton>
        <Badge color="primary" onClick={() => navigate(role === 'admin' ? `chatsAdmin/${data?.id}` : `chats/${data?.id}`, {replace: false})}>
